...
def main():
    ...
    dexhand = MyoHandR()
    chain = pk.build_chain_from_mjcf(open("/tmp/myohand_r_pk.xml").read())
    chain = chain.to(dtype=torch.float32, device=device)

    n_frames = demo["wrist_pos"].shape[0]
    print(f"retargeting {n_frames} frames")

    ref_model = mujoco.MjModel.from_xml_path(str(Path(__file__).resolve().parents[2] / "assets/hands/myohand_r_ulnaroot.xml"))
    dof_lo, dof_hi = [], []
    ...
    capsule_specs_raw = [
        ("2proxph_r_j1", 0.009, 0.02, [0.74364598, 0.64140698, 0.170361, -0.0810239]),
        ("midph2_r", 0.008, 0.012, [0.72704967, 0.6606737, 0.16878292, -0.08013336]),
        ("distph2_r", 0.007, 0.008, [0.72704967, 0.6606737, 0.16878292, -0.08013336]),
    ]
    capsule_specs = [
    ...
    # KNUCKLE (proximal joint) bodies -- same idea as fingertip contact
    # matching, but for the knuckles: pull each toward its OWN real captured
    # position relative to the mug (full vector, not just distance), so the
    # whole finger's path -- not just its tip -- stays on the correct side
    # of the mug rather than wrapping over the rim or clipping through
    proximal_names_set = {"thumb_proximal", "index_proximal", "middle_proximal", "ring_proximal", "pinky_proximal"}
    knuckle_body_to_key = {b: dexhand.to_hand(b)[0] for b in dexhand.body_names if dexhand.to_hand(b)[0] in proximal_names_set}
    ...
    idx_2proxph = dexhand.body_names.index("2proxph_r_j1")
    idx_5proxph = dexhand.body_names.index("5proxph_r_j1")
    idx_wrist = dexhand.body_names.index("lunate_r_j1")
    ...
    for it in range(1, max_iter + 1):
        ...
        beta_collision = 100.0  # RAISED from 20.0 -- max_pen was stuck around
        # 0.023-0.032 the whole run, barely moving, testing whether this is
        # under-weighting or a genuine structural conflict
        ...
        for b, col in tip_body_to_col.items():
            ...
            # real captured fingertip's OWN position in the mug's local frame --
            # its DIRECTION tells us which side (inside vs outside the wall) the
            # real finger was actually on; matching direction+magnitude together
            # (not just scalar radial distance) prevents the retargeted finger
            # from landing on the wrong side at the same distance from center,
            # which the old scalar-only formulation couldn't distinguish (this
            # was the cause of a finger visibly sitting INSIDE the mug's open
            # top rather than gripping its outer wall)
            real_tip_world = demo["mano_joints"][tip_names_order[col]].to(device)
            real_tip_local = (mug_rotmat.transpose(-1, -2) @ (real_tip_world - mug_pos).unsqueeze(-1)).squeeze(-1)
            real_tip_dir = torch.nn.functional.normalize(real_tip_local[..., :2], dim=-1)

            target_radial = mug_radius + tips_distance[:, col]
            target_xy = real_tip_dir * target_radial[:, None]

            contact_terms.append(((tip_local[..., :2] - target_xy) ** 2).sum(-1))
        contact_loss = torch.mean(torch.stack(contact_terms))
        beta_contact = 15.0

        # knuckle side-matching: pull each knuckle toward its real captured
        # position relative to the mug (full vector), keeping the whole
        # finger path -- not just the tip -- on the correct side
        knuckle_terms = []
        for b, key in knuckle_body_to_key.items():
            idx = dexhand.body_names.index(b)
            k_world = pk_world[:, idx]
            k_local = (mug_rotmat.transpose(-1, -2) @ (k_world - mug_pos).unsqueeze(-1)).squeeze(-1)
            real_k_world = demo["mano_joints"][key].to(device)
            real_k_local = (mug_rotmat.transpose(-1, -2) @ (real_k_world - mug_pos).unsqueeze(-1)).squeeze(-1)
            knuckle_terms.append(((k_local[..., :2] - real_k_local[..., :2]) ** 2).sum(-1))
        knuckle_side_loss = torch.mean(torch.stack(knuckle_terms))
        beta_knuckle_side = 10.0
    ...
    # swing/wander in an arc while the hand itself stayed coherent
        ...
        with torch.no_grad():
            ...
            # HARD table-height floor on the elbow. Optimization runs in the
            # RAW (uncorrected) frame; our axis-correction matrix C is a
            # permutation-style rotation where corrected-Z actually comes from
            # RAW-Y (confirmed: new_Z = old_Y) -- clamping raw-Z would silently
            # do nothing; the real floor constraint is on opt_wrist_pos[:, 1].
            opt_wrist_pos[:, 1].clamp_(min=0.0)
    ...
    out_path = Path(__file__).resolve().parents[2] / "assets/retargeted/1292e_rh_myohand.pkl"
